...
const DescriptionTab = ({ activity, themeContainer, themeText }: TabProps) => (
  <ScrollView style={[styles.tabContent, themeContainer]}>
    ...
    {activity.description.equipment.map((item, index) => (
      <Text key={index} style={[styles.bodyText, themeText]}>• {item}</Text>
    ))}

    <Text style={[styles.sectionTitle, themeText, styles.topMargin]}>Instructions</Text>
...
const WriteUpTab = ({ activity, themeContainer, themeText, isDark }: WriteUpProps) => {
  const data = activity.writeUp;

  const tableHeaderBg = isDark ? '#1e1e1e' : '#e0e0e0';
  const tableBorderColor = isDark ? '#333333' : '#cccccc';
  const cellStyle = [styles.tableCell, styles.standardColumnWidth, { borderRightWidth: 1, borderRightColor: tableBorderColor }];
    ...
    <ScrollView style={[styles.tabContent, themeContainer]}>
      ...
      <View style={styles.questionsContainer}>
        {data.questions.map((question: string, index: number) => (
          <Text key={index} style={[styles.bodyText, themeText, styles.italicText]}>
            {question}
          </Text>
        ))}
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator>
        <View style={[styles.tableContainer, { borderColor: tableBorderColor }]}>
          ...
          {/* Table Headers */}
          <View style={[styles.tableRow, { backgroundColor: tableHeaderBg, borderBottomWidth: 1, borderBottomColor: tableBorderColor }]}>
            <View style={[styles.tableCell, styles.firstColumnWidth, { borderRightWidth: 1, borderRightColor: tableBorderColor }]}><Text style={[styles.headerCellText, themeText]}>{data.tableHeaders[0]}</Text></View>
            <View style={cellStyle}><Text style={[styles.headerCellText, themeText]}>{data.tableHeaders[1]}</Text></View>
            <View style={cellStyle}><Text style={[styles.headerCellText, themeText]}>{data.tableHeaders[2]}</Text></View>
            <View style={cellStyle}><Text style={[styles.headerCellText, themeText]}>{data.tableHeaders[3]}</Text></View>
            <View style={lastCellStyle}><Text style={[styles.headerCellText, themeText]}>{data.tableHeaders[4]}</Text></View>
          </View>
          ...
          {data.tableRows.map((row: any, rowIndex: number) => {
            ...
            return (
              <View 
                key={rowIndex} 
                style={[styles.tableRow, { borderBottomWidth: isLastRow ? 0 : 1, borderBottomColor: tableBorderColor, minHeight: 80 }]}
              >
                <View style={[styles.tableCell, styles.firstColumnWidth, { borderRightWidth: 1, borderRightColor: tableBorderColor, backgroundColor: tableHeaderBg }]}>
                  <Text style={[styles.rowLabelText, themeText]}>{row.label}</Text>
                </View>
      ...
      <View style={styles.topMargin}>
        <Text style={[styles.sectionTitle, themeText]}>{data.discussionTitle}</Text>
...
const DiscussionTabContent = ({ activity, themeContainer, themeText, themeCard, isDark }: TabProps) => {
  
  const [userNotes, setUserNotes] = useState('');
  return (
  <ScrollView style={[styles.tabContent, themeContainer]}>
 <View style={[styles.topMargin]}>
        <Text style={[styles.sectionTitle, themeText]}>Your Discussion Observations</Text>
        <TextInput
          style={[
            styles.textArea, 
...
<View style={[styles.container, themeContainer]}>
      ...
      >
        ...
        <Tab.Screen name="Description">
          {(props) => <DescriptionTab activity={activity} themeContainer={themeContainer} themeText={themeText} />}
        </Tab.Screen>
       <Tab.Screen name="Write-up">
  {(props) => <WriteUpTab activity={activity} themeContainer={themeContainer} themeText={themeText} isDark={isDark} />}
</Tab.Screen>
        <Tab.Screen name="Discussion">
  {(props) => <DiscussionTabContent activity={activity} themeContainer={themeContainer} themeText={themeText} themeCard={themeCard} />}
</Tab.Screen>
      </Tab.Navigator>

      <TouchableOpacity 
        style={styles.fab} 
        onPress={() => router.push({ pathname: './activity-sensor-screen', params: { activityId: activity.id } })}
      >
...
const styles = StyleSheet.create({
  ...
  diagramImage: { width: '100%', height: 200, borderRadius: 8, marginTop: 10, backgroundColor: '#eee' },
  card: { padding: 16, borderRadius: 8, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
  lightCard: { backgroundColor: '#fff' },
  darkCard: { backgroundColor: '#1e1e1e' },
  cardTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 6 },
  formulaText: { fontSize: 18, fontFamily: 'monospace', fontStyle: 'italic', color: '#2196F3', paddingVertical: 4 },
  questionsContainer: { marginBottom: 20, paddingLeft: 4 },
  italicText: { fontStyle: 'italic', marginBottom: 4 },
  tableContainer: { borderWidth: 1, borderRadius: 4, overflow: 'hidden', marginTop: 10, marginBottom: 20 },
